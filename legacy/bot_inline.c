...
void bot_legacy_inline(const char *inline_query_data, json_object *update) {
    ...
    string_copy(inline_query, inline_query_data, sizeof(inline_query));

    if(sscanf(inline_query_data, "%hd", &page) == 1) {
        char page_buffer[8];
        snprintf(page_buffer, sizeof(page_buffer), "%d", page);

        string_copy(inline_query, &inline_query[strlen(page_buffer)], sizeof(inline_query));

        for(short s = 0; inline_query[s]; s++) {
            if(inline_query[s] == ' ' && inline_query[s + 1] == ' ') {
                string_copy(&inline_query[s], &inline_query[s + 1], sizeof(inline_query) - s);
                s--;
            }
        }
        ...
        for(short args = 0; args < 6; args++) {
            if(inline_query[0] == 'b' && !action) {
                action = 1;
                string_copy(inline_query, &inline_query[1], sizeof(inline_query));
                string_cat(arguments, "b", 2);
            } else if(inline_query[0] == 'n' && action == 1) {
                action = 2;
                string_copy(inline_query, &inline_query[1], sizeof(inline_query));
                string_cat(arguments, "n", 2);
            } else if(inline_query[0] == 't' && !action) {
                action = 3;
                string_copy(inline_query, &inline_query[1], sizeof(inline_query));
                string_cat(arguments, "t", 2);
            } else if(inline_query[0] == 'p' && !preview) {
                preview = 1;
                string_copy(inline_query, &inline_query[1], sizeof(inline_query));
                string_cat(arguments, "p", 2);
            } else if(inline_query[0] == 'a' && !autopaging) {
                autopaging = 1;
                string_copy(inline_query, &inline_query[1], sizeof(inline_query));
                string_cat(arguments, "a", 2);
            } else if(inline_query[0] == 'q' && !quickaccess) {
                quickaccess = 1;
                string_copy(inline_query, &inline_query[1], sizeof(inline_query));
                string_cat(arguments, "q", 2);
            }
        }

        if(inline_query[0]) {
            if(inline_query[0] == ' ') {
                string_copy(pre_query, &inline_query[1], sizeof(pre_query));

                CURL *encode_query = curl_easy_init();
                char *encoded_query = curl_easy_escape(encode_query, pre_query, 0);

                string_copy(query, encoded_query, sizeof(query));
    ...
    const char *inline_query_id = json_object_get_string(json_object_object_get(json_object_object_get(update, "inline_query"), "id"));
    ...
    if(page >= 1 && page <= 100) {
        int offset = json_object_get_int(json_object_object_get(json_object_object_get(update, "inline_query"), "offset"));
        ...
        if(json_object_get_type(data) == json_type_array) {
            while(json_object_array_get_idx(data, array)) {
                ...
                if(!action) {
                    ...
                    char *csc_method, *csc_key, *csc_width, *csc_height, *csc_mime = "", *csc_format= {0}, csc_image_url[512], csc_image_preview_url[512];
                    ...
                    if(csc_filetype) {
                        if(!strcmp(csc_filetype, "image/jpeg")) {
                            ...
                            string_copy(csc_image_preview_url, csc_preview_url ? csc_preview_url : SANKAKU_DPREVIEW_URL, sizeof(csc_image_preview_url));
                            ...
                            string_copy(csc_image_preview_url, SANKAKU_DPREVIEW_URL, sizeof(csc_image_preview_url));
                            csc_swidth = 150;
                            csc_sheight = 150;
                        }
                    } else {
                    ...
                    if(arguments[0])
                        string_cat(csc_button1, arguments, sizeof(csc_button1) - strlen(csc_button1));
                    if(pre_query[0])
    ...
    } else if(!inline_compare(inline_query_data, "original") || !inline_compare(inline_query_data, "post") || !inline_compare(inline_query_data, "book")) {
        command_args *arguments = command_args_parse(inline_query_data, 1, true);
        ...
        if(args) {
            ...
            if(csc_id) {
                if(!inline_compare(inline_query_data, "original")) {
                    const char *csc_preview_url = json_object_get_string(json_object_object_get(csc_data, "preview_url"));
    ...
    } else if(!inline_compare(inline_query_data, "short")) {
        command_args *arguments = command_args_parse(inline_query_data, 20, true);
        size_t args = command_args_count(arguments);
